    ...
    if (isset($_FILES['csv_file']) && $_FILES['csv_file']['error'] == 0) {
        ...
        if (strtolower($ext) === 'csv') {
            ...
            while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                // Assuming CSV format: Unit Code, Unit Name
                $unit_code = $data[0] ?? '';
                $unit_name = $data[1] ?? '';

                if (!empty($unit_code) && !empty($unit_name)) {
                    // Adjust table name 'units' and columns as per your DB schema
                    $sql = "INSERT INTO units (unit_code, unit_name) VALUES (?, ?) 
                            ON DUPLICATE KEY UPDATE unit_name = VALUES(unit_name)";
                    $stmt = $pdo->prepare($sql);
                    if ($stmt->execute([$unit_code, $unit_name])) {
                        $imported++;
                    }
                }
            }
    ...
    <div class="content">
      ...
      <form method="post" enctype="multipart/form-data">
        <div class="form-group">
          ...
          <small style="color: #666; display: block; margin-top: 5px;">Format: Unit Code, Unit Name</small>
        </div>
